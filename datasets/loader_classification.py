...
def susy(dataset_dir: Path) -> bool:
    ...
    Classification task. n_classes = 2.
    susy X train dataset (4000000, 18)
    susy y train dataset (4000000, 1)
    susy X test dataset  (1000000, 18)
    susy y test dataset  (1000000, 1)
    """
    dataset_name = 'susy'
    os.makedirs(dataset_dir, exist_ok=True)
    ...
    data = pd.read_csv(local_url, header=None)
...
def hepmass(dataset_dir: Path) -> bool:
    """
    HEPMASS dataset from UCI machine learning repository
    https://archive.ics.uci.edu/ml/datasets/HEPMASS

    Classification task. n_classes = 2.
    hepmass X train dataset (7000000, 28)
    hepmass y train dataset (7000000, 1)
    hepmass X test dataset  (3500000, 28)
    hepmass y test dataset  (3500000, 1)
    """
    ...
    local_train = os.path.join(dataset_dir, 'hepmass_train.csv.gz')
    local_test = os.path.join(dataset_dir, 'hepmass_test.csv.gz')

    if not os.path.isfile(local_train):
        logging.info(f'Started loading {dataset_name} train')
        retrieve(url_train, local_train)
    if not os.path.isfile(local_test):
        logging.info(f'Started loading {dataset_name} test')
        retrieve(url_test, local_test)
    logging.info(f'{dataset_name} is loaded, started parsing...')

    train = pd.read_csv(local_train)
    test = pd.read_csv(local_test)
    X_train = train.iloc[:, 1:].to_numpy(dtype=np.float32)
    y_train = train.iloc[:, 0].to_numpy(dtype=np.float32)
    X_test = test.iloc[:, 1:].to_numpy(dtype=np.float32)
    y_test = test.iloc[:, 0].to_numpy(dtype=np.float32)
...
def covtype_full(dataset_dir: Path) -> bool:
    """
    Cover type dataset (full split) from sklearn
    https://archive.ics.uci.edu/ml/datasets/covertype

    Binary classification (label > 3).
    covtype_full X train dataset (464809, 54)
    covtype_full y train dataset (464809, 1)
    covtype_full X test dataset  (116203, 54)
    covtype_full y test dataset  (116203, 1)
    """
    dataset_name = 'covtype_full'
    os.makedirs(dataset_dir, exist_ok=True)
...
def kddcup99(dataset_dir: Path) -> bool:
    """
    KDD Cup 1999 network intrusion dataset from sklearn

    Binary classification (normal vs attack).
    kddcup99 X train dataset (3919806, 41)
    kddcup99 y train dataset (3919806, 1)
    kddcup99 X test dataset  (979952, 41)
    kddcup99 y test dataset  (979952, 1)
    """
    ...
    logging.info(f'Started loading {dataset_name}')
    X, y = fetch_kddcup99(return_X_y=True, as_frame=False, percent10=False)
    logging.info(f'{dataset_name} is loaded, started parsing...')

    y = (y == b'normal.').astype(np.float32)
    X = pd.DataFrame(X)
...
def miniboone(dataset_dir: Path) -> bool:
    """
    MiniBooNE particle identification from UCI
    https://archive.ics.uci.edu/ml/datasets/MiniBooNE+particle+identification

    Classification task. n_classes = 2.
    miniboone X train dataset (~104000, 50)
    miniboone y train dataset (~104000, 1)
    miniboone X test dataset  (~26000, 50)
    miniboone y test dataset  (~26000, 1)
    """
    dataset_name = 'miniboone'
    os.makedirs(dataset_dir, exist_ok=True)

    url = 'https://archive.ics.uci.edu/ml/machine-learning-databases/00199/MiniBooNE_PID.txt'
    local_url = os.path.join(dataset_dir, 'MiniBooNE_PID.txt')
    if not os.path.isfile(local_url):
        logging.info(f'Started loading {dataset_name}')
        retrieve(url, local_url)
    logging.info(f'{dataset_name} is loaded, started parsing...')

    with open(local_url) as f:
        first_line = f.readline().strip().split()
        n_signal = int(first_line[0])
        n_background = int(first_line[1])

    data = pd.read_csv(local_url, skiprows=1, sep=r'\s+', header=None)
    X = data.to_numpy(dtype=np.float32)
    y = np.concatenate([np.ones(n_signal), np.zeros(n_background)]).astype(np.float32)
...
def poker(dataset_dir: Path) -> bool:
    """
    Poker Hand dataset from UCI
    https://archive.ics.uci.edu/ml/datasets/Poker+Hand

    Binary classification (hand >= 1, i.e. has a pair or better).
    poker X train dataset (25010, 10)
    poker y train dataset (25010, 1)
    poker X test dataset  (1000000, 10)
    poker y test dataset  (1000000, 1)
    """
    dataset_name = 'poker'
    os.makedirs(dataset_dir, exist_ok=True)
    ...
    local_train = os.path.join(dataset_dir, 'poker-hand-training-true.data')
    local_test = os.path.join(dataset_dir, 'poker-hand-testing.data')
    ...
    train = pd.read_csv(local_train, header=None)
    test = pd.read_csv(local_test, header=None)
    X_train = train.iloc[:, :-1].to_numpy(dtype=np.float32)
    y_train = (train.iloc[:, -1] >= 1).to_numpy(dtype=np.float32)
    X_test = test.iloc[:, :-1].to_numpy(dtype=np.float32)
    y_test = (test.iloc[:, -1] >= 1).to_numpy(dtype=np.float32)
